<?php
namespace jumaris\ares;

use Stichoza\GoogleTranslate\GoogleTranslate;
use jumaris\ares\ms\message;
...
class responder{
    ...
    public function bukaHadits($perawi,$angka){
        ...
        for($a=1;$a<50;$a++){  
            ...
            $file="book/hadits/".$perawi."-".$a.".json";
            if (file_exists($file) !== true){
                break;
            }
    ...
    public function random_hadits($message){
        ...
        $i=1;
        while($i<10){
            $file="book/hadits/".$prw."-".$this->myrandhadits().".json";
            if(file_exists($file)){  
    ...
    public function terjemah_ke($mess){
        $message=new message($mess,"ares/data/bahasa.php","bahasa");
        if ($message->oke){
            if ($message->value==""){
